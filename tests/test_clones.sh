...
curl -X POST 'http://localhost:4040/api/v0/settings/ipranges' -d 'ipRange=127.0.0.0/24'
sudo /usr/sbin/nivlheim_client
curl -X 'http://localhost:4040/api/v0/triggerJob/handleDNSchangesJob'
...
if sudo curl -s --cert /var/nivlheim/my.crt --key /var/nivlheim/my.key 'https://localhost/cgi-bin/secure/ping'; then
